...
# b = 한 번 체크한 값을 저장할 때 사용하는 비트수를 b
# c = 좌우 등 소리를 저장할 트랙 갯수인 채널 개수를 c
# s = 녹음할 시간(초)
""" 실제로, 일반적인 CD 음질(44.1KHz, 16bit, 스테레오)로 1초 동안 저장하려면
44100 * 16 * 2 * 1 bit의 저장공간이 필요한데 """
...
# 비트를 바이트로 환산하고 1024로 나눠서 KB로 만들고 1024로 나눠서 MB로 만든다,.
s_space = (h * b * c * s) / 8 / 1024 / 1024

print(f'{s_space :.1f} MB')
